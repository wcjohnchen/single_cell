...
<i>APA analysis</i>.  Expressed genes were matched to the terminal region (TR) genes in PolyADB-v3x-LR database (https://github.com/wcjohnchen/database).  A complete list of TR genes found in scRNA-seq samples can be viewed in the dotplot in analyze_data_sc_RNASeq.ipynb, and that in the scATAC-seq sample can be viewed in analyze_data_sc_ATACSeq.ipynb.  For scRNA-seq, among, there are.  For scATA-seq, among, there are.  
